...
function excercise11() {
  ...
  console.log(stackOfNumbers.pop()?.toFixed());
  ...
  console.log(stackOfStrings.pop()?.toUpperCase());
}
// TODO: compile and run the code
excercise11();
...
function excercise12() {
  ...
  function randomIndex(number: number): number {
  ...
  type TUser = { position: string };
  function getRandomObject(arr: TUser[]): TUser | undefined {
    if (arr.length === 0) return undefined;
    // const randomIndex = Math.floor(Math.random() * arr.length);
    return arr[randomIndex(arr.length)];
  }
  ...
  console.log(getRandomString(["one", "two", "three", "four", "five"]));
  console.log(
    getRandomObject([
      { position: "one" },
      { position: "two" },
  ...
  console.log(getRandomItem([{ position: "5" }, 5, "five", [5]]));
}
// TODO: compile and run the code
excercise12();
