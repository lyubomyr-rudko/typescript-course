...
function excercise11() {
  ...
  stackNum.push(2);
  stackNum.push(5);
  console.log(stackNum.pop()?.toFixed());

  const stackStr = new Stack<string>();
  stackStr.push('hello');
  stackStr.push('world');
  console.log(stackStr.pop()?.toUpperCase());
}
// TODO: compile and run the code
excercise11();
...
function excercise12() {
  ...
  console.log(random<number>([1, 2, 3]));
  console.log(random<string>(['hello', 'world', 'Serhii']));

  type TObj = { age: number };
  console.log(random<TObj>([{ age: 20 }, { age: 30 }, { age: 40 }]));
...
function excercise15() {
  ...
  const users = fetchUsers<TUser>();
  // + TODO: add type safety to the code to print the names of the users to console

  Array.isArray(users) && users.forEach((user) => console.log(user.name));
}
// TODO: compile and run the code
excercise15();
