...
function excercise12() {
  ...
  function numberReturner(num: number[]): number {
    return num[getRandomInt(num.length)];
  }
  function stringReturner(str: string[]): string {
    return str[getRandomInt(str.length)];
  }
  function objectReturner(obj: object[]): object {
...
function excercise15() {
  ...
  const users: UserObject[] = fetchUsers();
