...
function exercise17() {
  ...
  const student = new Student('Serhii', 32, 'qwerty');
...
function exercise19() {
  type TCommand = string | string[] | boolean;
  function formatCommandLine(command: TCommand) {
...
function exercise21() {
  ...
  const desktopWidget: TDesktopWidget & TWidgetWithSize & TWidget = {
    name: "widget",
    width: 10,
    height: 20,
    color: "red",
  ...
  const mobileWidget: TMobileWidget & TWidgetWithSize & TWidget = {
    name: "widget",
    width: 10,
    height: 20,
    color: "red",
