...
function exercise40() {
  ...
  if (fetchResult !== null) {
    ...
    userNames.forEach((name) => {
      ...
      // there is no type check in this code and it causes an error
...
function exercise41() {
  ...
  function printAllUserPropTypes() {
    ...
    const arrOfUserKeys = [];
    for (let t in user) {
      arrOfUserKeys.push(t);
    }
    arrOfUserKeys.forEach((el) => console.log(typeof el));
  }
...
function exercise42() {
  ...
  type TPhone = TProduct['warehouse']['phoneNumbers'] | undefined;
