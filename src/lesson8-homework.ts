...
function exercise40() {
  ...
  if (fetchResult !== null) {
    ...
    userNames.forEach((name) => {
      ...
      // inside callback we can get null, to fix this error we must use local variable or Optional chaining
...
function exercise42() {
  ...
  // + ODO: create a type TCoodinates that represents coordinates, using lookup type
...
function exercise46() {
  ...
  type FirstParameter<T> = T extends (...args: infer U) => any ? U[0] : never;
