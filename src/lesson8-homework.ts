(function () {
  ...
  function exercise41() {
    ...
    function printAllUserPropTypes(data: TUser) {
      ...
      const keys = Object.keys(data) as Array<keyof TUser>;
      keys.forEach((element: keyof TUser) => {
        console.log(element, typeof data[element]);
      });
    }
    // TODO: for each property of the user object, print its type using js typeof operator
    function printAllUserPropTypesUniversal<T extends TSomeObj>(data: T) {
      // TODO: get lis of own keys of the user object
      // TODO: iterate over the keys with foreach
      // TODO: console.log the typeof for each property
      const keys = Object.keys(data) as Array<keyof T>;
      keys.forEach((element: keyof T) => {
        if (
          typeof data[element] === 'object' &&
          !Array.isArray(data[element])
        ) {
          printAllUserPropTypes(data[element]);
        }
        console.log(element, typeof data[element]);
      });
    }
    printAllUserPropTypes(user);
    printAllUserPropTypesUniversal(user);
  ...
  function exerciseExtra1() {
    ...
    function areAnagrams<T extends string, F extends (s: string) => string>(
      s1: T,
      s2: T,
      sorter: F,
    ): boolean {
      if (s1.length !== s2.length) return false;
      if (sorter(s1) === sorter(s2)) return true;
      return false;
    }

    console.log(areAnagrams('listen', 'silent', prepareString));
    console.log(areAnagrams('abc', 'cba', prepareString));
    console.log(areAnagrams('abc', 'cbd', prepareString));
