...
function exercise40() {
  ...
  if (fetchResult !== null) {
    ...
    userNames.forEach((name) => {
      ...
      let result = localFetchResult.find((obj: GroupDocument) => obj.name === name);
      if (result) {
...
function exercise41() {
  ...
  function printAllUserPropTypes() {
    ...
    const keys = Object.keys(user) as TUserKey[];
    // TODO: iterate over the keys with foreach
    keys.forEach((key: TUserKey) => console.log(typeof user[key]));
...
function exercise42() {
  ...
  type TPhoneNumber = TProduct["warehouse"]["phoneNumbers"][0];
...
function exercise43() {
  ...
  type TPerson = typeof user;
  type TKeyOfPerson = keyof TPerson;
...
function exercise46() {
  ...
  type FirstParameter<T> = T extends (...args: infer R) => any ? R[0] : never;
...
function exerciseExtra1() {
  ...
  console.assert(areAnagrams("listen", "silent"), "listen and silent are not anagrams");
  console.assert(areAnagrams("abc", "cba"), "abc and cba are not anagrams");
  console.assert(areAnagrams("abc", "cbd"), "abc and cbd are not anagrams");
